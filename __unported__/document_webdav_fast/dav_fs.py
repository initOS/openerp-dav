...
import urlparse
import urllib
from cache_cursor import CacheCursor
...
class openerp_dav_handler(dav_interface):
    ...
    def get_cr(self, uri, allow_last=False):
        ...
        (db, pool, cr) = self._cache_cr.get()
        if not cr or cr.closed:
            db, pool = pooler.get_db_and_pool(dbname)
            cr = db.cursor()
            self._cache_cr.put((db, pool, cr))
        return cr, uid, pool, dbname, uri2

    def clean_up_connection(self):
        _, _, cr = self._cache_cr.get()
        if cr:
            cr.close()
        self._cache_cr.delete()
